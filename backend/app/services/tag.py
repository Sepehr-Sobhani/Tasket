...
class TagService:
    ...
    async def get_tag_by_id(self, tag_id: int, user_id: int) -> Tag | None:
    ...
    async def create_tag(self, tag_data: TagCreate, creator_id: int) -> Tag | None:
        ...
        # Create tag
        tag = Tag(**tag_data.model_dump(), creator_id=creator_id)
    ...
    async def update_tag(
        self, tag_id: int, tag_data: TagUpdate, user_id: int
    ) -> Tag | None:
    ...
    async def add_tag_to_task(
        self, tag_id: int, task_id: int, user_id: int
    ) -> TaskTag | None:
        ...
        stmt = select(TaskTag).where(
            TaskTag.tag_id == tag_id, TaskTag.task_id == task_id
        )
        result = await self.db.execute(stmt)
        existing_tag = result.scalar_one_or_none()
        ...
        task_tag = TaskTag(tag_id=tag_id, task_id=task_id)
        ...
        logger.info(
            "Tag added to task", tag_id=tag_id, task_id=task_id, user_id=user_id
        )
        return task_tag
    ...
    async def get_task_tags(self, task_id: int, user_id: int) -> list[Tag]:
