...
class EpicService:
    ...
    async def get_project_epics(
        ...
        stmt = select(
            ...
            Epic.is_active == True
        ).group_by(Epic.id).order_by(Epic.created_at.desc()).offset(skip).limit(limit)
    ...
    async def get_epic_by_id(
        self,
        epic_id: int,
        user_id: int
    ) -> Optional[Epic]:
        """Get a specific epic if user has access"""
        stmt = select(Epic).join(ProjectMember).where(
            Epic.id == epic_id,
            ProjectMember.user_id == user_id,
            ProjectMember.is_active == True
        )
    ...
    async def create_epic(
        ...
        logger.info("Epic created", epic_id=epic.id, creator_id=creator_id, project_id=epic_data.project_id)
        return epic
    ...
    async def add_task_to_epic(
        ...
        logger.info("Task added to epic", task_id=task_id, epic_id=epic_id, user_id=user_id)
        return task
    ...
    async def remove_task_from_epic(
        ...
        logger.info("Task removed from epic", task_id=task_id, epic_id=epic_id, user_id=user_id)
        return task
    ...
    async def search_epics(
        ...
        if query:
            stmt = stmt.where(
                Epic.title.ilike(f"%{query}%") | 
                Epic.description.ilike(f"%{query}%")
            )
