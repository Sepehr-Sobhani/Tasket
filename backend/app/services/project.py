...
class ProjectService:
    ...
    async def get_user_projects(
        ...
        ).group_by(Project.id).offset(skip).limit(limit)
    ...
    async def get_dashboard_stats(self, user_id: int) -> DashboardStats:
        ...
        user_projects_stmt = select(Project.id).join(ProjectMember).where(
            ProjectMember.user_id == user_id,
            ProjectMember.is_active == True
        )
        ...
        unique_members_stmt = select(func.count(func.distinct(ProjectMember.user_id))).where(
    ...
    async def add_project_member(
        ...
        logger.info("Project member added", project_id=project_id, user_id=user_id, role=role)
    ...
    async def remove_project_member(
        ...
        if member:
            ...
            logger.info("Project member removed", project_id=project_id, user_id=user_id)
            return True
    ...
    async def check_user_project_access(
        ...
        project_id: int,
        user_id: int,
        required_role: Optional[ProjectMemberRole] = None
    ) -> bool:
