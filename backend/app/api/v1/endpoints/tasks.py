...
async def create_task(
    *,
    ...
    """Create new task"""
    task_service = TaskService(session)
    task = await task_service.create_task(task_data=task_in, creator_id=current_user.id)

    if not task:
        raise HTTPException(
            status_code=403,
            detail="You must be a member of the project to create tasks",
        )
...
async def update_task(
    *,
    ...
    task = await task_service.update_task(
        task_id=task_id, task_data=task_in, user_id=current_user.id
    )
...
async def get_tasks_by_project(
    *,
    ...
    if tasks is None:
        raise HTTPException(
            status_code=403, detail="You must be a member of the project to view tasks"
        )
