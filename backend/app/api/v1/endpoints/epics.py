...
from app.schemas.epic import EpicCreate, Epic as EpicSchema
...
async def get_epics(
    ...
    stmt = select(Epic).join(Epic.project).join(ProjectMember).where(
...
async def get_epics_by_project(
    *,
    ...
    if not project_member:
        raise HTTPException(status_code=403, detail="You must be a member of the project to view epics")
...
async def create_epic(
    *,
    ...
    if not project_member:
        raise HTTPException(status_code=403, detail="You must be a member of the project to create epics")
