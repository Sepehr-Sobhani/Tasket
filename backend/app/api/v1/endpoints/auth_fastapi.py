...
async def oauth_test():
    ...
    try:
        ...
        return {
            ...
            "github_client_id": getattr(github_client, "client_id", None)
            if github_client
            else None,
            "google_client_id": getattr(google_client, "client_id", None)
            if google_client
            else None,
        }
    except Exception as e:
...
async def oauth_callback(
    request: Request, provider: str, session: AsyncSession = Depends(get_async_session)
):
    ...
    try:
        ...
        frontend_url = settings.FRONTEND_URL or "http://localhost:3000"
        error_url = (
            f"{frontend_url}/auth/callback?error=auth_failed&error_description={str(e)}"
        )
        return RedirectResponse(url=error_url)


async def get_or_create_oauth_user(
    session: AsyncSession, provider: str, user_info: dict, token: dict
) -> User:
    ...
    stmt = select(OAuthAccount).where(
        OAuthAccount.oauth_name == provider, OAuthAccount.account_id == account_id
    )
